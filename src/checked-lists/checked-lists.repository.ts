...
export interface CheckedListsRepository {
  findAll();
  findOne(date: string);
  create(createCheckedListDto: CheckedList);
...
@Injectable()
export class CheckedListsMongoRepository implements CheckedListsRepository {
  constructor(
    @InjectModel(CheckedLists.name)
  ...
  async create(createCheckedListDto: CheckedList) {
    try {
